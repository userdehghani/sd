...
export interface LoginWithPassKeyOutput {
  accessToken: string;
  session: Session;
}

export class LoginWithPassKeyCommand {
  constructor(
  ...
  async execute(input: LoginWithPassKeyInput): AsyncResult<LoginWithPassKeyOutput, DomainError> {
    logger.info("Executing LoginWithPassKeyCommand", { email: input.email });
    ...
    if (!user.hasAuthProvider(AuthProvider.PASSKEY) || !user.passKeyCredential) {
      logger.warn("User does not have PassKey enabled", { userId: user.id.getValue() });
      return Err(
        new AuthenticationError("PassKey not enabled for this user", ErrorCode.PASSKEY_INVALID)
      );
    }

    // Parse stored credential
    const storedCredential = JSON.parse(user.passKeyCredential);

    // Verify PassKey
    const verifyResult = await this.passKeyService.verifyAuthentication(
      input.credential,
      storedCredential.publicKey
    );
    if (!verifyResult.success) {
      logger.error("Error verifying PassKey", { error: verifyResult.error });
      return verifyResult;
    }
    if (!verifyResult.value) {
      logger.warn("Invalid PassKey", { userId: user.id.getValue() });
      return Err(
        new AuthenticationError("Invalid PassKey", ErrorCode.PASSKEY_INVALID)
      );
    }
    ...
    logger.info("User logged in successfully with PassKey", { 
      userId: user.id.getValue(),
      sessionId: session.id.getValue() 
    });
